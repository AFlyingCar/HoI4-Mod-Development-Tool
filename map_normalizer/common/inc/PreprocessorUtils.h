...
# define UNIQUE_NAME(PREFIX) CONCAT(CONCAT(CONCAT(PREFIX, _L),__LINE__), \
...
# define ON_STATIC_INIT_BEGIN_BLOCK \
    ON_STATIC_INIT_BEGIN_BLOCK_IMPL(EXPAND(UNIQUE_NAME(___ON_STATIC_INIT___)))

# define ON_STATIC_INIT_BEGIN_BLOCK_IMPL(OBJ_NAME)  \
    struct OBJ_NAME {                          \
        OBJ_NAME()

# define ON_STATIC_INIT_END_BLOCK } UNIQUE_NAME(___ON_STATIC_INIT_INSTANCE___)
